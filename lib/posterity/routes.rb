module Posterity
  module Routes
    def blog_for(path, *args)
      ...
      match "#{path}/:year/:month/:day" => "posterity/posts#index", :as => "#{path}_archive", :resources => options[:resources].to_s
      match "#{path}/:year/:month" => "posterity/posts#index", :as => "#{path}_archive", :resources => options[:resources].to_s
      match "#{path}/:year" => "posterity/posts#index", :as => "#{path}_archive", :resources => options[:resources].to_s
      ...
      match "#{path}/permalink/:permalink" => "posterity/posts#show", :as => "#{path.to_s.singularize}_permalink", :resources => options[:resources].to_s
    end
  end
end
